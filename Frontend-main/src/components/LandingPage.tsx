...
function generateMockData() {
  ...
  nodes[0].riskScore = 95; nodes[0].type = 'mule';
  nodes[0].flaggedPatterns = ['Rapid fund cycling', 'Cross-border smurfing', 'Velocity spike'];
  nodes[1].riskScore = 88; nodes[1].type = 'mule';
  nodes[2].riskScore = 92; nodes[2].type = 'shell';
  nodes[3].riskScore = 85; nodes[3].type = 'mule';
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    for (let j = 0; j < numLinks; j++) {
      ...
      if (target !== i) {
        links.push({ source: nodes[i].id, target: nodes[target].id, value: Math.floor(Math.random() * 100000) + 5000 });
      }
    }
  }
